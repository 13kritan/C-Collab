...
export const useDocumentAudit = ({ documentId }) => {
  ...
  // Helper to push logs with a "System Log" aesthetic
  const pushLog = (type, message) => {
    setAuditLog((prev) => [
      ...
      ...prev.slice(0, 49), // Keep last 50 entries to prevent memory bloat
    ]);
  };
  ...
  return {
    ...
    // Export socket so your editor can use it to emit changes
